...
async function run() {
  ...
  for (const route of routes) {
    const pageReport = { route, screenshots: [], console: [], networkFailures: [], navigations: [] };
    for (const width of viewports) {
      ...
      page.on('requestfailed', (req) => { pageReport.networkFailures.push({ url: req.url(), method: req.method(), failure: req.failure() && req.failure().errorText }); });

      const url = `${base.replace(/\/$/, '')}${route}`;
      try {
        const resp = await page.goto(url, { waitUntil: 'networkidle', timeout: 45000 });
        const navTimingJson = await page.evaluate(() => JSON.stringify(performance.getEntriesByType('navigation'))).catch(() => null);
        pageReport.navigations.push({ width, status: resp ? resp.status() : null, url: page.url(), timing: navTimingJson ? JSON.parse(navTimingJson) : null });

        const safeName = route === '/' ? 'index' : route.replace(/[^a-zA-Z0-9]/g,'_');
        const screenshotPath = path.join(artifactsDir, `${safeName}-${width}.png`);
        await page.screenshot({ path: screenshotPath, fullPage: true });
        pageReport.screenshots.push(screenshotPath.replace(/\\/g, '/'));

        const html = await page.content();
        const htmlPath = path.join(artifactsDir, `${safeName}-${width}.html`);
