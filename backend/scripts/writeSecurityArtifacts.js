...
async function main() {
  ...
  if (!runId) {
    console.error('Usage: node scripts/writeSecurityArtifacts.js <runId>');
    process.exit(2);
  }
  ...
  for (const p of pages) {
    const pageKey = sanitize(p.normalizedPath || p.baseline?.normalizedPath || p.candidate?.normalizedPath || '/');
    const result = {
